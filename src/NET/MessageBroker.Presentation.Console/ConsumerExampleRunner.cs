...
public class ConsumerExampleRunner
{
    private readonly IExampleInputProvider _inputProvider;
    private readonly IExampleOutputHandler _outputHandler;

    public ConsumerExampleRunner(IExampleInputProvider inputProvider, IExampleOutputHandler outputHandler)
    {
        _inputProvider = inputProvider;
        _outputHandler = outputHandler;
    }

    public async Task RunAsync(IMessageExample<ConsumerExampleStep> example, CancellationToken cancellationToken)
    {
        if (example is MessageBroker.Example.CrossCut.Examples.Consumer.BaseConsumerExample baseConsumer)
        {
            void Handler(string consumerKey, string message)
                => System.Console.WriteLine($"[Consumed by {consumerKey}] {message}");
            baseConsumer.MessageReceived += Handler;
            try
            {
                await RunStepsAsync(example, cancellationToken);
            }
            finally
            {
                baseConsumer.MessageReceived -= Handler;
    ...
    private async Task RunStepsAsync(IMessageExample<ConsumerExampleStep> example, CancellationToken cancellationToken)
    {
        while (!example.IsComplete)
        {
            ...
            if (!int.TryParse(stepInput, out int stepNum) || !Enum.IsDefined(typeof(ConsumerExampleStep), stepNum))
            {
                await _outputHandler.WriteOutputAsync("Invalid step selection. Please try again.", cancellationToken);
                continue;
            }
            var step = (ConsumerExampleStep)stepNum;
    ...
    private async Task ShowStepMenuAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var step in Enum.GetValues(typeof(ConsumerExampleStep)))
        {
